...
import { getUserSubscriptionById, updateUserSubscriptionById } from 'apiSdk/user-subscriptions';
...
function UserSubscriptionEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<UserSubscriptionInterface>(
    () => (id ? `/user-subscriptions/${id}` : null),
  ...
  const handleSubmit = async (values: UserSubscriptionInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateUserSubscriptionById(id, values);
      mutate(updated);
      resetForm();
