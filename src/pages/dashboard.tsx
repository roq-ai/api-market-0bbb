...
import { getApiDocumentations } from 'apiSdk/api-documentations';
import { ApiDocumentationInterface } from 'interfaces/api-documentation';
import { getApiListings } from 'apiSdk/api-listings';
import { ApiListingInterface } from 'interfaces/api-listing';
import { getApiReviews } from 'apiSdk/api-reviews';
import { ApiReviewInterface } from 'interfaces/api-review';

import { ApiRatingListPage as ShowCaseList } from 'pages/api-ratings';

function DashboardPage() {
  const { session } = useSession();

  const apiDocumentationStat = useSWR<PaginatedInterface<ApiDocumentationInterface>>(
    () => `/api-documentations`,
    () => getApiDocumentations({ limit: 1, offset: 0 }),
  );

  const apiListingStat = useSWR<PaginatedInterface<ApiListingInterface>>(
    () => `/api-listings`,
    () => getApiListings({ limit: 1, offset: 0 }),
  );

  const apiReviewStat = useSWR<PaginatedInterface<ApiReviewInterface>>(
    () => `/api-reviews`,
    () => getApiReviews({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Api Documentations',
      result: apiDocumentationStat?.data?.totalCount,
      isLoading: apiDocumentationStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Api Listings',
      result: apiListingStat?.data?.totalCount,
      isLoading: apiListingStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Api Reviews',
      result: apiReviewStat?.data?.totalCount,
      isLoading: apiReviewStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`api-ratings`} passHref legacyBehavior>
                  <Button
