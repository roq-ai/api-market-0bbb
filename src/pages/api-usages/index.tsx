...
type ColumnType = ColumnDef<ApiUsageInterface, unknown>;

interface ApiUsageListPageProps {
...
export function ApiUsageListPage(props: ApiUsageListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ApiUsageInterface>>(
    () => `/api-usages?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ApiUsageInterface) => {
    if (hasAccess('api_usage', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/api-usages/view/${row.id}`);
    }
  };
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="api_usage" />
        </Flex>

        {hasAccess('api_usage', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/api-usages/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
